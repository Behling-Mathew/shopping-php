...
/* 
 * This model is for the product image uploads
 */
...
// Obtain thumbnail image information from the images table based on invId
function getThumbnail($invId) {
  $db = acmeConnect();
  $sql = 'SELECT imgId, invId, imgName, imgPath, imgDate FROM images WHERE imgName LIKE "%-tn%" AND invId = :invId';
  $stmt = $db->prepare($sql);
  $stmt->bindValue(':invId', $invId, PDO::PARAM_INT);
  $stmt->execute();
